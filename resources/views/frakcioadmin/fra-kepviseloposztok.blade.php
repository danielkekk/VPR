            ...
            <div class="content">
                ...
                if(isset($kepviseloposztok)) {
                    foreach ($kepviseloposztok as $kp) {
                        echo '<span style="color: red; font-weight: bold;">' . $kp->name . '['.$kp->kovetok_szama.']&nbsp;('.$kp->ev.'-'.$kp->honap.'-'.$kp->nap.')</span>&nbsp;&nbsp;&nbsp;<a href="'.url("/fra-editkepviseloposzt/{$kp->id}").'">[SZ]</a>&nbsp;&nbsp;<a href="'.url("/fra-deletekepviseloposzt/{$kp->id}").'">[X]</a><br>';
                    }
                }
                ?>

                <!--<br><br><br>
                <form id="ujfrakcio-form" action="{{ url('/fra-createfrakcio') }}" method="POST">
                    @csrf
